...
grid <- expand.grid(k=k_seq,e=e_seq, xm=c(0.01,0.02,0.04,0.08,seq(from=0.1,to=1,by=.1)), xf=c(0.01,0.02,0.04,0.08,seq(from=0.1,to=1,by=.1)), m=1, p=c(0.5), h=c(F), r=c(.20), c=c(1), rep=1:10)
...
run <- function(setup, ceed){
  ...
  # indDF[,genes] <- round(runif(min=0,max=1,n=setup$nInd*4),2) # for free innovateProp
  indDF[,genes] <- round(runif(min=0,max=1,n=setup$nInd*4)) # for fixed innovateProp
  # Assign values for females from mothers
  # indDF[ indDF[,"S"]==1 ,"innovateProp"] <- round(rowMeans(indDF[ indDF[,"S"]==1 , c("f1","f2")]), 2)  # for free innovateProp
  indDF[ indDF[,"S"]==1 ,"innovateProp"] <- round(rowMeans(indDF[ indDF[,"S"]==1 , c("f1","f2")]), 1) # for fixed innovateProp
  # Assign values for males from fathers
  indDF[ indDF[,"S"]==0 ,"innovateProp"] <- round(rowMeans(indDF[ indDF[,"S"]==0 , c("m1","m2")]))#, 2)
  indDF[ indDF[,"S"]==0 ,"innovateProp"] <- round(rowMeans(indDF[ indDF[,"S"]==0 , c("m1","m2")]), 1)
  ...
  femIL <- mean(tail(fems, 1000))
  memIL <- mean(tail(mems, 1000))
  
  # return(cbind(indDF, setup, payIL, paySL, diffpatIL, diffpatSL))
  return(cbind(indDF, setup, femIL, memIL))
} # END RUN
